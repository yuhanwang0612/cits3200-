...
const [researchers, publications, links, summaries, quality] = await Promise.all([
  read("uwa_accounting_researchers.json"),
  read("uwa_accounting_publications_ranked.json"),
  read("uwa_accounting_researcher_publications.json"),
  read("uwa_accounting_researcher_summary.json"),
  read("uwa_accounting_data_quality.json"),
]);
...
for (const summary of summaries) {
  assert(
    Number(summary.publication_count_in_pure_collection) === (countByResearcher.get(summary.researcher_id) || 0),
