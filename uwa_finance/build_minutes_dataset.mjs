...
const [staffPayload, publicationPayload, rankingText] = await Promise.all([
  fs.readFile(path.join(INPUT_DIR, "uwa_finance_staff.json"), "utf8").then(JSON.parse),
  fs.readFile(path.join(INPUT_DIR, "uwa_finance_publications.json"), "utf8").then(JSON.parse),
...
const researchers = staffPayload.records.map((researcher, index) => ({
  researcher_id: `uwa-finance-${String(index + 1).padStart(3, "0")}`,
  ...researcher,
...
const dataQuality = {
  ...
  scope: "University of Western Australia Finance proof of concept",
  ...
  pure_extracted_profile_count: staffPayload.extracted_finance_profile_count,
  ...
  pure_extracted_record_count: publicationPayload.extracted_finance_publication_count,
  pure_count_reconciles: publicationPayload.count_reconciles,
  publication_details_without_visible_finance_affiliation:
    publicationPayload.publication_details_without_visible_finance_affiliation.length,
  ...
  warnings: [
    ...(publicationPayload.count_reconciles
      ? []
      : [
          `Pure reported ${publicationPayload.reported_organisation_publication_count} organisation outputs while ${publicationPayload.extracted_finance_publication_count} were extracted; investigate before claiming completeness.`,
        ]),
    ...(publicationPayload.publication_details_without_visible_finance_affiliation.length
      ? [`${publicationPayload.publication_details_without_visible_finance_affiliation.length} RSS-listed publications do not expose a Finance organisation link on their detail page; RSS membership was retained as authoritative.`]
      : []),
    "The Pure Finance organisation output list is not guaranteed to contain every publication for every current researcher.",
    `${unmatchedPublicationIds.length} organisation outputs do not link to one of the current Finance profiles; the collection includes historical organisation output.`,
...
await Promise.all([
  fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_researchers.json"), `${JSON.stringify(researchers, null, 2)}\n`),
  fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_publications_ranked.json"), `${JSON.stringify(publications, null, 2)}\n`),
  fs.writeFile(
    path.join(OUTPUT_DIR, "uwa_finance_researcher_publications.json"),
    `${JSON.stringify(researcherPublications, null, 2)}\n`,
  ),
  fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_researcher_summary.json"), `${JSON.stringify(researcherSummary, null, 2)}\n`),
  fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_data_quality.json"), `${JSON.stringify(dataQuality, null, 2)}\n`),
  writeCsv(
    researchers,
    [
  ...
  writeCsv(
    ...
    "uwa_finance_researcher_publications.csv",
  ),
  writeCsv(
    researcherSummary,
    [
    ...
    "uwa_finance_researcher_summary.csv",
  ),
]);
