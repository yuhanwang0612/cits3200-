...
const ORG_RSS = "https://research-repository.uwa.edu.au/en/organisations/finance-2/persons/?format=rss";
...
function parseProfile(html, profileUrl, personType) {
  ...
  const finance = affiliations.find((item) => item.organisation_url.includes("/en/organisations/finance-2/"));
  ...
  const mapped = academicLevel(finance.title);
  const reviewReasons = [];
  if (!mapped.level) reviewReasons.push("academic level requires client confirmation");
  if (/assistant professor/i.test(finance.title)) reviewReasons.push("UWA Assistant Professor mapping requires confirmation");
  if (/teaching|honorary|emeritus|adjunct|visitor|contractor|fellow/i.test(finance.title)) {
  ...
  return {
    name_display: name,
    academic_title: finance.title || "Not stated",
...
const payload = {
  ...
  extracted_finance_profile_count: records.length,
  count_reconciles: reportedProfileCount === records.length,
  failed_profile_requests: failures,
  records,
};

await fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_staff.json"), `${JSON.stringify(payload, null, 2)}\n`, "utf8");
const columns = [
...
await fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_staff.csv"), `${rows.join("\n")}\n`, "utf8");

console.log(JSON.stringify({
  reported_profiles: reportedProfileCount,
  candidate_profiles: candidateUrls.length,
  extracted_finance_profiles: records.length,
