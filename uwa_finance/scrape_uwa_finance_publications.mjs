...
const ORG_PAGE = "https://research-repository.uwa.edu.au/en/organisations/finance-2/";
const ORG_RSS = "https://research-repository.uwa.edu.au/en/organisations/finance-2/publications/?format=rss";
...
function parsePublication(html, articleUrl) {
  ...
  const financeAffiliationObserved = organisationUrls.some((url) => url.includes("/en/organisations/finance-2/"));
  ...
  return {
    ...
    finance_affiliation_observed: financeAffiliationObserved,
  };
}
...
const payload = {
  ...
  collection_method: "All publication URLs exposed by the official paginated Finance publications RSS feed, enriched from individual Pure publication pages",
  ...
  extracted_finance_publication_count: records.length,
  count_reconciles: reportedPublicationCount === records.length,
  failed_publication_requests: failedRequests,
  publication_details_without_visible_finance_affiliation: records
    .filter((record) => !record.finance_affiliation_observed)
    .map((record) => record.article_url),
  records,
};
await fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_publications.json"), `${JSON.stringify(payload, null, 2)}\n`, "utf8");
...
const columns = [
  ...
  "finance_affiliation_observed", "source_url", "harvested_at",
];
...
await fs.writeFile(path.join(OUTPUT_DIR, "uwa_finance_publications.csv"), `${rows.join("\n")}\n`, "utf8");

console.log(JSON.stringify({
  reported_organisation_publications: reportedPublicationCount,
  discovered_recent_urls: publicationUrls.length,
  extracted_finance_publications: records.length,
  count_reconciles: payload.count_reconciles,
  failed_requests: failedRequests.length,
  details_without_visible_finance_affiliation: records.filter((record) => !record.finance_affiliation_observed).length,
}, null, 2));
