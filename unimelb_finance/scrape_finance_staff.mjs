...
// Use the official Department of Finance tag, matching the Accounting collector's department-based scope.
const STAFF_PAGE = "https://fbe.unimelb.edu.au/about/academic-staff?queries_tags_query=4895953";
...
try {
  ...
  const candidates = rows
    .map((row) => ({
      ...
      department: clean(row.department) || "Department of Finance",
  ...
  const excludedRecords = candidates.filter((row) => !matchesDepartment(row.department, "Department of Finance"));
  const normalized = candidates
    .filter((row) => matchesDepartment(row.department, "Department of Finance"))
    .map((row) => {
  ...
  const payload = {
    ...
    filter_rule: "Department field contains 'Department of Finance' (case-insensitive)",
    records: normalized,
  };
