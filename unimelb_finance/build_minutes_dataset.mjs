...
const [staffPayload, publicationPayload, rankingText] = await Promise.all([
  fs.readFile(path.join(INPUT_DIR, "unimelb_finance_staff.json"), "utf8").then(JSON.parse),
  fs.readFile(path.join(INPUT_DIR, "unimelb_finance_publications.json"), "utf8").then(JSON.parse),
...
const researchers = staffPayload.records.map((researcher, index) => ({
  researcher_id: `unimelb-finance-${String(index + 1).padStart(3, "0")}`,
  ...researcher,
...
const dataQuality = {
  ...
  scope: "University of Melbourne Department of Finance proof of concept",
  ...
  warnings: [
    ...
    "The Minerva Finance collection is not guaranteed to contain every publication for every current researcher.",
...
await Promise.all([
  fs.writeFile(path.join(OUTPUT_DIR, "unimelb_finance_researchers.json"), `${JSON.stringify(researchers, null, 2)}\n`),
  fs.writeFile(path.join(OUTPUT_DIR, "unimelb_finance_publications_ranked.json"), `${JSON.stringify(publications, null, 2)}\n`),
  fs.writeFile(
    path.join(OUTPUT_DIR, "unimelb_finance_researcher_publications.json"),
    `${JSON.stringify(researcherPublications, null, 2)}\n`,
  ),
  fs.writeFile(path.join(OUTPUT_DIR, "unimelb_finance_researcher_summary.json"), `${JSON.stringify(researcherSummary, null, 2)}\n`),
  fs.writeFile(path.join(OUTPUT_DIR, "unimelb_finance_data_quality.json"), `${JSON.stringify(dataQuality, null, 2)}\n`),
  writeCsv(
    researchers,
    [
    ...
    "unimelb_finance_researchers.csv",
  ),
  writeCsv(
    publications,
    [
    ...
    "unimelb_finance_publications_ranked.csv",
  ),
  writeCsv(
    ...
    "unimelb_finance_researcher_publications.csv",
  ),
  writeCsv(
    researcherSummary,
    [
    ...
    "unimelb_finance_researcher_summary.csv",
  ),
]);
