...
recovered here from OpenAlex author search, because `info/orcid.py`,
`info/crossref.py` and `info/openalex.py` all skip a researcher with
no ORCID, and without this step three of the five retrieval stages would do
nothing at all for UNSW. The ISSNs have to come from `enrichment/openalex.py`.
...
def add_openalex_ids(records, verbose=True):
    ...
    UNSW publishes no ORCID anywhere, and everything in info/ starts with
    ...
    permanently. Recording the author id here means info/openalex.py can
...
def parse_publications(soup, person):
    ...
    for item in soup.select(PUB_ITEM):
        ...
        record = blank_pub(
            ...
            # enrichment/openalex.py, which is why that module taking the ISSN
            ...
            link=link,
            source=SOURCE_NAME,
            # Carried beyond the base schema, as info/orcid.py already
