...
# --- the "other files" this adapter hands off to -------------------------
# ADJUST: these modules don't exist until you build them. The calls below
# assume a uniform interface:
#     each info/ module exposes       backfill(pubs) -> pubs
#     each enrichment/ module exposes enrich(pubs)   -> pubs
# Rename to match whatever you actually implement. (info.openalex and
# enrichment.openalex are the same source doing two different jobs, hence
# the aliases.)
from info import crossref, openalex as openalex_ids
from enrichment import abdc, clarivate, scimago, openalex as openalex_metrics
...
# final output/uq/  — two levels up from base_scrapers/uq.py
SLUG = "uq"
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "final output" / SLUG
...
# --- 5. shared info + enrichment -----------------------------------------

def enrich(pubs, verbose=True):
    """Hand the raw publications off to the shared info/ and enrichment/
    modules. info/ fills in identifiers eSpace didn't give us; enrichment/
    attaches journal rankings and metrics.

    NOTE: UQ already gets ORCIDs straight from eSpace (see add_orcids), so
    info/orcid is deliberately not called here. A uni that doesn't get
    ORCIDs natively would add `records = orcid.backfill(records)` upstream.
    """
    # info/ — backfill DOIs eSpace didn't provide
    pubs = crossref.backfill(pubs)            # ADJUST: fn name
    pubs = openalex_ids.backfill(pubs)        # ADJUST: fn name

    # enrichment/ — attach rankings + metrics
    pubs = scimago.enrich(pubs)               # ADJUST: SJR + quartile
    pubs = abdc.enrich(pubs)                  # ADJUST: ABDC A*/A/B/C rank
    pubs = clarivate.enrich(pubs)             # ADJUST: JCR impact factor
    pubs = openalex_metrics.enrich(pubs)      # ADJUST: citation counts

    if verbose:
        print(f"  enriched {len(pubs)} publications")
    return pubs


# --- 6. derived entity tables --------------------------------------------

def build_journals(pubs):
    """One row per unique journal seen in the publications.

    ADJUST: `CARRY` lists the columns copied onto each journal row. Add the
    field names your enrichment modules write onto each pub (e.g.
    "quality_rank", "sjr_quartile", "jcr_if") so the rankings flow through.
    """
    CARRY = ["journal", "journal_canonical", "publisher", "issns"]
    seen, journals = set(), []
    for p in pubs:
        key = p.get("journal_canonical") or p.get("journal")
        if not key or key in seen:
            continue
        seen.add(key)
        journals.append({k: p.get(k) for k in CARRY})
    return journals


def build_harvest(pubs):
    """One row per data source that fed this uni's dataset — mirrors the
    'one row per source' shape of the old harvest.csv.

    ADJUST: list the sources you actually run, and whatever per-source
    fields you want to record.
    """
    sources = ["UQ eSpace", "Crossref", "OpenAlex", "ABDC JQL 2025",
               "Scimago", "Clarivate JCR"]
    today = date.today().isoformat()
    return [{"university": UNIVERSITY, "source": s, "harvested": today}
            for s in sources]
...
def _serialize(value):
    """CSV cells can't hold lists — join them; leave everything else alone."""
...
def _fieldnames(rows):
    """Union of keys across all rows, in first-seen order — so a column that
    only some rows have still gets written (never silently dropped)."""
    fields = []
    for r in rows:
        for k in r:
            if k not in fields:
...
def _write_csv(rows, path):
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        w.writeheader()
        for r in rows:
            w.writerow({k: _serialize(v) for k, v in r.items()})


def write_output(records, pubs, journals, harvest, verbose=True):
    """Write the five deliverables into final output/uq/."""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    _write_csv(pubs,     OUTPUT_DIR / f"{SLUG}_publications.csv")
    _write_csv(records,  OUTPUT_DIR / f"{SLUG}_staff.csv")
    _write_csv(journals, OUTPUT_DIR / f"{SLUG}_journals.csv")
    _write_csv(harvest,  OUTPUT_DIR / f"{SLUG}_harvest.csv")
    (OUTPUT_DIR / f"{SLUG}_harvest.json").write_text(
        json.dumps(harvest, indent=2, ensure_ascii=False), encoding="utf-8")
    if verbose:
...
def collect(verbose=True):
    """Run the full UQ pipeline: scrape the site, hand off to info/ and
    enrichment/, then write the output files.

    Still returns (records, pubs) so a central runner can import and call
    this without re-reading them off disk.
    """
    records = scrape_staff(verbose)          # uni-specific: get staff
    add_source_ids(records, verbose)         # uni-specific: eSpace author id
    add_orcids(records, verbose)             # uni-specific: ORCID from eSpace
    pubs = fetch_publications(records, verbose)  # uni-specific: publications

    pubs = enrich(pubs, verbose)             # call the other files
    journals = build_journals(pubs)          # derive journal table
    harvest = build_harvest(pubs)            # derive harvest log

    write_output(records, pubs, journals, harvest, verbose)  # make the output
    return records, pubs
