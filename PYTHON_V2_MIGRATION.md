...
- `pipeline/`: validation, publication/identity review, versioned raw runs, team-schema
...
5. Keep every person from the official target rosters. Store uncertain
   publication/identity records in SQLite without gating staff inclusion.
6. Publish approved publication relationships and the complete staff roster as
   an immutable revision, then atomically replace `data/current.json`.
